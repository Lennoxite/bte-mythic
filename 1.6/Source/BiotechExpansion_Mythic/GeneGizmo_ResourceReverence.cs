	...
	public class GeneGizmo_ResourceReverence : GeneGizmo_Resource
	{
		...
		public override GizmoResult GizmoOnGUI(Vector2 topLeft, float maxWidth, GizmoRenderParms parms)
		{
            ...
            if (MapGizmoUtility.LastMouseOverGizmo is Command_Ability command_Ability && gene.Max != 0f)
            {
                ...
                {
                    if (effectComp is CompAbilityEffect_ReverenceCost compAbilityEffect_ReverenceCost && compAbilityEffect_ReverenceCost.Props.reverenceCost > float.Epsilon)
                    {
                        Rect rect = barRect.ContractedBy(3f);
                        float width = rect.width;
                        float num3 = gene.Value / gene.Max;
                        rect.xMax = rect.xMin + width * num3;
                        float num4 = Mathf.Min(compAbilityEffect_ReverenceCost.Props.reverenceCost / gene.Max, 1f);
                        rect.xMin = Mathf.Max(rect.xMin, rect.xMax - width * num4);
                        GUI.color = new Color(1f, 1f, 1f, num2 * 0.7f);
                        GenUI.DrawTextureWithMaterial(rect, GeneGizmo_ResourceReverence.ReverenceCostTex, null);
