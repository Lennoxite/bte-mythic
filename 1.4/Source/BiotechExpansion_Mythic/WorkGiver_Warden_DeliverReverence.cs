...
namespace BTE_MY
{
	...
	public class WorkGiver_Warden_DeliverReverence : WorkGiver_Warden
	{
		...
		public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			Gene_Reverence gene_Reverence = ((genes != null) ? genes.GetGene(BTEMY_GeneDefOf.BTEMy_ReveredEmbodiment) : null) as Gene_Reverence;
			...
			if (this.ReverencePackAlreadyAvailableFor(prisoner))
			{
				return null;
			}
		...
		private bool ReverencePackAlreadyAvailableFor(Pawn prisoner)
		{
			if (prisoner.carryTracker.CarriedCount(BTEMY_ThingDefOf.BTEMy_ReverenceFuel) > 0)
			{
				return true;
			}
			if (prisoner.inventory.Count(BTEMY_ThingDefOf.BTEMy_ReverenceFuel) > 0)
			{
				return true;
			}
			...
			if (room != null)
			{
				...
				{
					if (regions[i].ListerThings.ThingsOfDef(BTEMY_ThingDefOf.BTEMy_ReverenceFuel).Count > 0)
					{
						return true;
					}
				}
